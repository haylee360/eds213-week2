...
eggs_sum <- eggs_data %>%
  ...
  summarise(Egg_count = max(Egg_num)) %>% # decided to use max number of eggs, but could have counted number of rows
  ungroup()
...
snowcover_data_fixed <- snowcover_data_fixed %>%
  mutate(Date2 = ifelse(is.na(Date2), as_date("08/06/06"), Date))
```
